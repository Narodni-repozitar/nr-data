...
def create_api_blueprint(app):
    """Create DatasetsFile blueprint."""
    blueprint = app.extensions["datasets"].resource_files.as_blueprint()
    ...
    funcs = [
        ...
        if k.startswith("init_addons_datasets_file") and callable(v)
    ]
    for func in funcs:
...
def init_addons_datasets_file_published_service(state):
    """Init app."""
    app = state.app
    ext = app.extensions["datasets"]
